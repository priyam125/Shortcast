...
import { UPLOAD_AUDIO, SPLASH_API, UPLOAD_POST, THREAD_POSTS } from '../Utils/apiroutes'
...
const AddthreadPost = () => {
    ...
    useEffect(() => {
        const getthreadposts = async () => {
            ...
            try {
                const response = await axios.get(`${THREAD_POSTS}${id}/`, {
                    mode: "cors",
                    headers: {
                ...
                setparentgenre(response.data.data.main_post[0].genre)
    ...
    const postallrecordedaudio = () => {
        ...
        generatedurls.forEach((urls, index) => {
            let post_thread_heading = `Thread`
            ...
            if (data[index] !== undefined) {
                ...
                if (data[index].value !== "") {
                    post_thread_heading = data[index].value
                }
            }
    ...
    return (
        <div className="addpostwrapper1 flex justify-center items-center mt-1 min-h-full overflow-y-scroll ">
            ...
            >
                <div className=" md:h-80 md:w-80 h-72 w-72  relative p-2">
                    <h2 className="text-xl font-extrabold">Are you sure you want to delete whole audio recorded ? </h2>
